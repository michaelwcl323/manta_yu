#!/usr/bin/env python3
"""
Monitor CPU and bandwidth usage on CloudLab nodes and save results.
"""
...
def resolve_output_dir(benchmark_dir: Path, explicit_output_dir: Optional[str]) -> Path:
    ...
    return (benchmark_dir / "logs").resolve()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Record remote CPU/bandwidth usage"
    )
    parser.add_argument(
        "--settings",
        default="cloudlab_settings.json",
        help="Path to cloudlab settings JSON",
    )
    parser.add_argument(
        "--output-dir",
        default=None,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0,
        help="Total monitoring duration seconds (<=0 means run until stopped)",
    )
    parser.add_argument(
        "--exclude-iface-regex",
