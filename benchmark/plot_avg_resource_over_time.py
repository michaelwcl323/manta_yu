...
def main():
    parser = argparse.ArgumentParser(description="Plot avg CPU and bandwidth% over time.")
    ...
    cpu_points = list(zip(ts, cpu))
    bw_points = list(zip(ts, bw))

    cpu_png = out_dir / "avg_cpu_usage_over_time.png"
    bw_png = out_dir / "avg_bandwidth_used_percent_over_time.png"

    draw_chart(cpu_png, "Average CPU Usage Over Time (All Nodes)", "CPU Usage (%)", cpu_points, (31, 119, 180))
    draw_chart(
        bw_png,
